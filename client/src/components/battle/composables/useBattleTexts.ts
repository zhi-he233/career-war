...
export function useBattleTexts(params: UseBattleTextsParams) {
  const {
    room,
    playerId,
    isDuoMode,
  ...
  const winner = computed(() =>
    room.value.players.find((p) => p.id === room.value.winnerId),
  );
  ...
  const winnerText = computed(() => {
    ...
    if (isRogueliteMode.value) return winner.value?.id === playerId ? "挑战成功" : "挑战失败";
    if (isPveMode.value) return winner.value?.id === playerId ? "胜利" : "失败";
  ...
  const opponentPanelHint = computed(() =>
    isRogueliteMode.value ? "点击敌人头像选择攻击目标" : "点击头像选择攻击目标",
  );
