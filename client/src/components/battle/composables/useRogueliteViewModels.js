...
import { ROGUELITE_BOSS_ABILITY_REWARDS, ROGUELITE_BOSSES, ROGUELITE_CHARACTER_SKILL_REWARDS, ROGUELITE_ENEMIES, ROGUELITE_GROWTH_REWARDS, ROGUELITE_STAGE_SCALING, ROGUELITE_STARTER_REWARDS, } from "@career-war/shared";
/** All roguelite-related ViewModels extracted from BattlePage.vue to keep the page component manageable. */
export function useRogueliteViewModels(room, me, playerId) {
    ...
    const ROGUELITE_BOSS_DISPLAY_DRAFTS = ROGUELITE_BOSS_ABILITY_REWARDS;
    const ROGUELITE_SKILL_DISPLAY_DRAFTS = ROGUELITE_CHARACTER_SKILL_REWARDS;
    const ROGUELITE_GROWTH_REWARD_TYPES = new Set(ROGUELITE_GROWTH_DISPLAY_DRAFTS.map((reward) => reward.type));
    const ROGUELITE_BOSS_REWARD_TYPES = new Set(ROGUELITE_BOSS_DISPLAY_DRAFTS.map((reward) => reward.type));
    const ROGUELITE_SKILL_REWARD_TYPES = new Set(ROGUELITE_SKILL_DISPLAY_DRAFTS.map((reward) => reward.type));
    function createPerkDisplay() {
        const result = {};
        ...
        if (heavyPunch)
            result.heavy_punch = heavyPunch;
        return result;
    }
    function createSkillDisplay() {
        const result = {};
    ...
    // ── enemy / boss helpers ──
    function rogueliteEnemyMechanicSkills(id) {
        const boss = ROGUELITE_BOSSES.find((item) => item.id === id);
        if (boss)
            return [...boss.skills];
        const enemy = ROGUELITE_ENEMIES.find((item) => item.id === id);
        if (!enemy || enemy.id === "normal")
            return [];
        return [...enemy.skills];
    }
    function rogueliteEnemyTypeLabel(player) {
    ...
    const rogueliteEnemyInfo = computed(() => {
        if (!isRogueliteMode.value || room.value.phase !== "battle")
            return undefined;
    ...
    const currentBossSkills = computed(() => {
        if (!currentBossPlayer.value)
            return [];
        if (currentBossPlayer.value.rogueliteEnemyInfo?.skillNames?.length)
            return currentBossPlayer.value.rogueliteEnemyInfo.skillNames;
        if (!currentBossPlayer.value.rogueliteBossId)
            return [];
    ...
    const rogueliteStageType = computed(() => {
        ...
        if (liveStageType)
            return liveStageType;
        const stage = rogueliteState.value?.stage ?? 1;
        if (stage % ROGUELITE_STAGE_SCALING.bossInterval === 0)
            return "boss";
        if (stage % ROGUELITE_STAGE_SCALING.bossInterval === 2 && stage >= 5)
            return "elite";
        return "normal";
    });
    const rogueliteStageTypeLabel = computed(() => {
        if (rogueliteStageType.value === "boss")
            return "Boss 战";
        if (rogueliteStageType.value === "elite")
            return "精英关";
        return "普通关";
    });
    // ── reward type helpers ──
    function isBossRewardType(type) { return ROGUELITE_BOSS_REWARD_TYPES.has(type); }
    function isGrowthRewardType(type) { return ROGUELITE_GROWTH_REWARD_TYPES.has(type); }
    function isRogueliteSkillRewardType(type) { return ROGUELITE_SKILL_REWARD_TYPES.has(type); }
    function rogueliteRewardRarity(type) {
    ...
    function rogueliteRewardTags(type) {
        if (isBossRewardType(type))
            return ["Boss", "Endgame"];
        if (isRogueliteSkillRewardType(type))
            return ["Skill", "Build"];
    ...
    function buildRogueliteBossVM() {
        ...
        if (state) {
            if (state.charge > 0)
                chips.push({ text: `蓄力层数：${state.charge}`, kind: "normal" });
            if (state.enraged)
                chips.push({ text: "狂暴中", kind: "enraged" });
            if (state.guarding)
                chips.push({ text: "架盾中", kind: "guarding" });
            if (state.bloodSacrificeUsed)
                chips.push({ text: "血祭已触发", kind: "normal" });
            if (currentBossPlayer.value.rogueliteBossId === "boss_god_berserker") {
                ...
                if (state.dyingAfterAttack)
                    chips.push({ text: "濒死一击中", kind: "enraged" });
            }
        }
        return {
    ...
    function buildRogueliteEnemyVM() {
        const info = rogueliteEnemyInfo.value;
        if (!info || currentBossPlayer.value || room.value.phase !== "battle")
            return undefined;
    ...
    function buildRogueliteResourcesVM() {
        const m = me.value;
        if (!m || room.value.phase !== "battle")
            return undefined;
        const res = {};
        const hasFateTokens = m.roguelitePerkStacks?.["fate_tokens"];
        if (hasFateTokens)
            res.fateTokens = { current: m.rogueliteFateTokens ?? 0, max: 3 };
        if (m.roguelitePerkStacks?.["low_roll_charge"])
            res.lowRollCharge = m.rogueliteLowRollCharge ?? 0;
        if (m.roguelitePerkStacks?.["lucky_floor"])
            res.consecutiveLowRolls = { current: m.rogueliteConsecutiveLowRolls ?? 0, max: 2 };
        if (m.roguelitePerkStacks?.["shield_overload"])
            res.shieldOverloadUsed = m.rogueliteShieldOverloadUsed ?? false;
        return Object.keys(res).length > 0 ? res : undefined;
    }
    function buildRogueliteEnemyTraitsVM() {
    ...
    function buildRoguelitePerksVM() {
        return {
            growth: rogueliteGrowthPerks.value.map((p) => ({
                id: p.perkId, name: p.def.name, level: p.level, description: p.def.perLevelDesc, category: "growth",
            })),
            skills: rogueliteCharacterSkills.value.map((s) => ({
                id: s.skillId, name: s.def.name, level: s.level, description: s.def.perLevelDesc, category: "skill",
            })),
            boss: rogueliteBossPerks.value.map((p) => ({
                id: p.perkId, name: p.def.name, level: p.level, description: p.def.perLevelDesc, category: "boss",
            })),
        };
    }
    function buildRogueliteRewardPhaseVM() {
    ...
    function buildRogueliteContinuePhaseVM() {
        return {
    ...
    const roguelitePanelVM = computed(() => ({
    ...
    function roguelitePhaseText() {
        if (room.value.phase === "reward")
            return isBossRewardPhase.value ? "选择Boss能力" : "选择奖励";
