...
function createMockUser(username: string): ProfileUser {
...
function createMockProgress(seed: number): ProfileProgress {
...
function createMockPvpStats(seed: number): ProfilePvpStats {
...
function createMockRogueliteStats(seed: number): ProfileRogueliteStats {
  return {
...
function createMockCareerStats(seed: number): ProfileCareerStats {
  return {
    favoriteCareer: seededPick(CAREER_IDS, seed),
    bestWinRateCareer: seededPick(CAREER_IDS, seed + 3),
    recentCareers: Array.from({ length: 3 + (seed % 3) }, (_, i) =>
      seededPick(CAREER_IDS, seed + i * 7),
    ),
  };
}

function createMockAchievements(seed: number): ProfileAchievement[] {
 ...
 *   return res.json();
 */
export function createMockProfile(username: string): PlayerProfile {
  const seed = seededFromUsername(username);
  return {
...
export const GUEST_PROFILE: PlayerProfile = createMockProfile("游客骰王");
