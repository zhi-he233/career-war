...
export const ROGUELITE_MAP_RULES = {
    ...
    typePattern: ["normal", "event", "rest", "shop", "elite", "boss", "normal", "shop", "event", "elite", "boss", "reward", "event", "rest", "boss"],
    cycleLayers: {
        1: ["normal"],
        2: ["normal", "event"],
        3: ["normal", "rest"],
...
export function getRogueliteCycleStage(stage) {
...
export function getRogueliteMapLayerCount(stage) {
    const fixedLayer = ROGUELITE_MAP_RULES.cycleLayers[getRogueliteCycleStage(stage)];
    if (fixedLayer)
        return fixedLayer.length;
    if (stage > 0 && getRogueliteCycleStage(stage) === ROGUELITE_STAGE_SCALING.bossInterval)
        return 1;
    if (stage <= 1)
        return 1;
...
export function getRogueliteMapNodeType(stage, branch, total) {
    const fixedLayer = ROGUELITE_MAP_RULES.cycleLayers[getRogueliteCycleStage(stage)];
    if (fixedLayer)
        return fixedLayer[branch] ?? fixedLayer[0] ?? "normal";
    if (stage > 0 && getRogueliteCycleStage(stage) === ROGUELITE_STAGE_SCALING.bossInterval)
        return "boss";
    if (stage <= 1 || total <= 1)
...
export function getRogueliteMapNodeEnemyTemplateId(stage, type, branch) {
    const cycleStage = getRogueliteCycleStage(stage);
    if (type === "normal") {
        if (cycleStage <= 2)
            return "normal_training_dummy";
        return branch % 3 === 1 ? "normal_gambler" : branch % 3 === 2 ? "normal_shield_breaker" : "normal";
    }
    if (type === "elite")
        return branch % 2 === 0 ? "elite_iron_skin" : "elite_berserker";
    return undefined;
}
