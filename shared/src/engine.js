...
import { applyLastStandOutcome, createNonAttackSummonerSkillOutcome, createSummonerRollChange } from "./summonerSkillOutcomes.js";
...
import { createSummonerChangedRollResolution, getAllowedRollDecisionAction, normalizeRollDecisionChoice } from "./rollDecisions.js";
...
export function startGame(room, ctx) {
    ...
    for (const player of room.players) {
        ...
        player.maxHp = character.maxHp;
        player.hp = getInitialHp(player.characterId, character.maxHp);
        player.shield = 0;
        player.zhaoZilongHitCount = 0;
        player.flameMarks = 0;
        player.guarding = false;
...
export function rollForActivePlayer(room, playerId, ctx, requesterId) {
    ...
    if (dicePerks.applied) {
        ...
        if (dicePerks.fateTokens !== undefined)
            actor.rogueliteFateTokens = dicePerks.fateTokens;
        if (dicePerks.consecutiveLowRolls !== undefined)
            actor.rogueliteConsecutiveLowRolls = dicePerks.consecutiveLowRolls;
        if (dicePerks.lowRollCharge !== undefined)
            actor.rogueliteLowRollCharge = dicePerks.lowRollCharge;
        if (dicePerks.shield !== undefined)
            actor.shield = dicePerks.shield;
...
export function confirmRollDecision(room, playerId, decisionId, choice, ctx, summonerSkillId, selfDamageAmount, requesterId) {
    ...
    if (normalizedChoice === "roguelite_skill") {
        const skillId = selectedAction.skillId ?? "";
...
function confirmSummonerSkill(room, actor, target, decision, rollEvent, ctx, requestedSkillId) {
    ...
    if (skillId === "last_stand") {
        const outcome = applyLastStandOutcome(resolveSkill(actor.characterId, decision.currentRoll, room.previousFinalDamage, actor.hp, actor.maxHp, target.hp, target.maxHp, actor.guarding ?? false, target.flameMarks ?? 0, { useOptionalCharacterSkill: false }));
        actor.summonerSkillCooldown = getSummonerSkillCooldown(actor, 3);
        room.pendingRollDecision = undefined;
        return finishDecisionAction(room, actor, target, outcome, rollEvent, ctx);
    }
...
function confirmChangedRollSummonerSkill(room, actor, target, decision, rollEvent, ctx, skillId, rollChange) {
    actor.summonerSkillCooldown = getSummonerSkillCooldown(actor, 3);
    ...
    room.pendingRollDecision = undefined;
    const outcome = resolveSkill(actor.characterId, rollChange.nextRoll, room.previousFinalDamage, actor.hp, actor.maxHp, target.hp, target.maxHp, actor.guarding ?? false, target.flameMarks ?? 0, { useOptionalCharacterSkill: false });
    return finishDecisionAction(room, actor, target, outcome, rollEvent, ctx);
}
    ...
    if (room.gameMode === "pve_roguelite" && actor.isBot && actor.rogueliteBossId) {
        ...
        if (actor.rogueliteBossId === "boss_boxer_king") {
            ...
            if (firstRoll === 1 || firstRoll === 2) {
                bossState.charge = (bossState.charge ?? 0) + 1;
                outcome.damage = 0;
            ...
            const charge = bossState.charge ?? 0;
...
export function beginGuardCheckIfNeeded(room, actorId, controllerId) {
    if (room.pendingGuardCheck)
        return room.pendingGuardCheck.actorId === actorId;
    if (room.guardCheckCompletedForActorId === actorId)
        return false;
...
export function resolveGuardCheck(room, actorId, ctx) {
    ...
    if (!pending || pending.actorId !== actorId)
        throw new Error("当前没有需要结算的架盾判定");
    const actor = getPlayerOrThrow(room, actorId);
    if (actor.characterId !== "mountain_shield")
        throw new Error("只有山盾需要架盾判定");
    if (!actor.guarding)
        throw new Error("山盾当前不在架盾状态");
...
function applyTurnStartCharacterEffects(room, actor, events, ctx) {
...
function hasMountainShieldGroupArmor(room, target) {
    return room.players.some((player) => {
        if (player.characterId !== "mountain_shield" || !player.guarding || player.isDead)
            return false;
        if (room.gameMode === "duo_2v2")
            return Boolean(player.teamId && target.teamId && player.teamId === target.teamId);
...
function getInitialHp(characterId, maxHp) {
