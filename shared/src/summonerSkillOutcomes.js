export function createFirstAidOutcome(roll) {
    return {
        damage: 0,
        healing: roll,
        shieldGain: 0,
...
export function createIronWallOutcome(roll) {
    return {
        damage: 0,
        healing: 0,
        shieldGain: roll,
...
export function applyLastStandOutcome(baseOutcome) {
    return {
        ...baseOutcome,
...
export function createNonAttackSummonerSkillOutcome(skillId, roll) {
    if (skillId === "first_aid")
        return createFirstAidOutcome(roll);
    if (skillId === "iron_wall")
        return createIronWallOutcome(roll);
    return undefined;
}
export function createSummonerRollChange(skillId, currentRoll, options = {}) {
    ...
    if (skillId === "fate_reroll") {
        if (typeof options.reroll !== "number")
            throw new Error("命运重掷需要新的骰点");
        return {
